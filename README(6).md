...
自动抽取阶段只读取原始PDF。阶段六至阶段九的验收成果只在自动输出已经持久化后用于独立评价，不反向参与章节定位、事件识别或字段抽取。
...
- 下一步有价值的工作是冻结当前规则，并选择另一家公司进行盲测，报告章节定位召回率、候选事件召回率、字段准确率和人工复核率。
